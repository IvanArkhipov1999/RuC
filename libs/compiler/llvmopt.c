		...
		switch (node_get_type(&child))
		{
			...
			case TExprend:
				...
				if (info->slice_depth == 0)
				{
					...
					// TODO: тут какая-то непонятная проблема, нужно ли это или нет
					// если есть, то вызов функций неправильно работает
					// если нет, то в умножении матриц вырезки неправильно переставляются
